...
## caveats
JSON is a data-interchange format so not the tool for reliable client/server data sync unique id numbers for tasks added, but most (all?) of these problems would be solved with a proper db backend instead of a json textfile.
